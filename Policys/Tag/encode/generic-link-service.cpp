...
//s
  shared_ptr<lp::LcdTag> lcdTag = netPkt.getTag<lp::LcdTag>();                                           //by yin - lcdInsertTag 
  if (lcdTag != nullptr) {
    lpPacket.add<lp::LcdTagField>(*lcdTag);
    NFD_LOG_DEBUG("LcdTag in generic-link-service in encodeLpFields:" << *lcdTag);
  }
  else {
    NFD_LOG_DEBUG("LcdTag in generic-link-service in encodeLpFields: lost!");
  }
//e

//s
  shared_ptr<lp::IdeaTag> ideaTag = netPkt.getTag<lp::IdeaTag>();                                        //by yin - IdeaTag
  if (ideaTag != nullptr) {
    lpPacket.add<lp::IdeaTagField>(*ideaTag);
    NFD_LOG_DEBUG("IdeaTag in generic-link-service in encodeLpFields:" << *ideaTag);
  }
  else {
    NFD_LOG_DEBUG("IdeaTag in generic-link-service in encodeLpFields: lost!");
  }
//e

//s
  shared_ptr<lp::RanTag> ranTag = netPkt.getTag<lp::RanTag>();                                           //by yin - RanTag
  if (ranTag != nullptr) {
    lpPacket.add<lp::RanTagField>(*ranTag);
    NFD_LOG_DEBUG("RanTag in generic-link-service in encodeLpFields:" << *ranTag);
  }
  else {
    NFD_LOG_DEBUG("RanTag in generic-link-service in encodeLpFields: lost!");
  }
//e

//s
  shared_ptr<lp::PopTag> popTag = netPkt.getTag<lp::PopTag>();                                           //by yin - PopTag
  if (popTag != nullptr) {
    lpPacket.add<lp::PopTagField>(*popTag);
    NFD_LOG_DEBUG("PopTag in generic-link-service in encodeLpFields:" << *popTag);
  }
  else {
    NFD_LOG_DEBUG("PopTag in generic-link-service in encodeLpFields: lost!");
  }
//e
}
...
GenericLinkService::decodeInterest(const Block& netPkt, const lp::Packet& firstPkt)
{
  ...
  if (firstPkt.has<lp::LcdTagField>()) {                                                                  //by yin - LcdTag
      interest->setTag(make_shared<lp::LcdTag>(firstPkt.get<lp::LcdTagField>()));   
  }
  if (firstPkt.has<lp::IdeaTagField>()) {                                                                 //by yin - IdeaTag
      interest->setTag(make_shared<lp::IdeaTag>(firstPkt.get<lp::IdeaTagField>()));                         
  }
  if (firstPkt.has<lp::RanTagField>()) {                                                                  //by yin - RanTag
      interest->setTag(make_shared<lp::RanTag>(firstPkt.get<lp::RanTagField>()));                         
  }
  if (firstPkt.has<lp::PopTagField>()) {                                                                  //by yin - PopTag
      interest->setTag(make_shared<lp::PopTag>(firstPkt.get<lp::PopTagField>()));                       
  }
...
GenericLinkService::decodeData(const Block& netPkt, const lp::Packet& firstPkt)
{
  ...
  if (firstPkt.has<lp::LcdTagField>()) {                                                              //by yin - LcdTag
      data->setTag(make_shared<lp::LcdTag>(firstPkt.get<lp::LcdTagField>()));                         //by yin
  }

  if (firstPkt.has<lp::IdeaTagField>()) {                                                             //by yin - IdeaTag
      data->setTag(make_shared<lp::IdeaTag>(firstPkt.get<lp::IdeaTagField>()));                       //by yin
  }

  if (firstPkt.has<lp::RanTagField>()) {                                                              //by yin - RanTag
      data->setTag(make_shared<lp::RanTag>(firstPkt.get<lp::RanTagField>()));                         //by yin
  }
  
  if (firstPkt.has<lp::PopTagField>()) {                                                              //by yin - PopTag
      data->setTag(make_shared<lp::PopTag>(firstPkt.get<lp::PopTagField>()));                         //by yin
  }
