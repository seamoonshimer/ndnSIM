...
// Idea (1.get IdeaTag | 2.IdeaTag-- | 3.set IdeaTag | 4.check to cache)
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
   shared_ptr<lp::IdeaTag> idea = data.getTag<lp::IdeaTag>();
   auto node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
   auto ti = ns3::Simulator::Now().ToDouble (ns3::Time::S);
   if(idea !=nullptr){
     int idean = *idea;
       idean--;
       data.setTag(make_shared<lp::IdeaTag>(idean));
       shared_ptr<Data> dataCopyWithoutTag = make_shared<Data>(data);
       dataCopyWithoutTag->removeTag<lp::HopCountTag>();
...
//         std::cout<<ti<<" forwarder::onIncomingdata(caching) "<<data.getName()<<" IdeaTag is "<<idean<<" in Node* "<<node->GetId()<< std::endl;
       }

       if(node->GetId() != 0 && idean == 0){
         if (m_csFromNdnSim == nullptr)
           m_cs.insert(*dataCopyWithoutTag);
         else
           m_csFromNdnSim->Add(dataCopyWithoutTag);
...
//         std::cout<<ti<<" forwarder::onIncomingdata(caching) "<<data.getName()<<" IdeaTag* is "<<idean<<" in Node "<<node->GetId()<< std::endl;
       ...
       if(node->GetId() !=0 && idean != 0){
//         std::cout<<ti<<" forwarder::onIncomingdata(no caching) "<<data.getName()<<" IdeaTag* is "<<idean<<" in Node "<<node->GetId()<< std::endl;
       }
   }
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
