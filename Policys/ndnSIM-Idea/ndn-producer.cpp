...
// Idea (1.get IdeaTag | 2.calculate cache hop | 3.set IdeaTag as hopnum) 
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
  shared_ptr<lp::IdeaTag> ideaTag = interest->getTag<lp::IdeaTag>();
  if(ideaTag != nullptr){      
    
    int hopNum = 0;
    hopNum = 6 - (*ideaTag-1) / 50; 
    data->setTag(make_shared<lp::IdeaTag>(hopNum));
    
    int ti = Simulator::Now().ToDouble (Time::S);                                                                                                                   
//    std::cout << ti << " producer return data " << data->getName() << " add IdeaTag " << data->getTag<lp::IdeaTag>()->get() << std::endl; 
  }
//--------------------------------------------------------------------------------------------------------------------------------------------------------------

//  NS_LOG_INFO("node(" << GetNode()->GetId() << ") responding with Data: " << data->getName());
  NS_LOG_INFO("Responding with Data: " << data->getName() << " in node " << GetNode()->GetId());         //LOG
