...
// Lcd cs hit (1.set Tag 0 | 2.log)                                                                                                                     
//------------------------------------------------------------------------------------------------------------------------------------------------------
  auto node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());                                                                                     
  auto ti = ns3::Simulator::Now().ToDouble (ns3::Time::S);                                                                                              
  data.setTag(make_shared<lp::LcdTag>(0));                                                                                                              
//  std::cout<<ti<<" forwarder::cache hit "<<data.getName()<<" Node "<<node->GetId()<<" LcdTag set "<<*data.getTag<lp::LcdTag>()<< std::endl;
//------------------------------------------------------------------------------------------------------------------------------------------------------
...
// Lcd (1.check Tag&Node | 2.cache or nocache)
//-------------------------------------------------------------------------------------------------------------------------------------------------------
  shared_ptr<lp::LcdTag> lcd = data.getTag<lp::LcdTag>();
  auto node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  auto ti = ns3::Simulator::Now().ToDouble (ns3::Time::S);
  ...
  if(lcd != nullptr){
      ...
      if(*lcd != 1){
        ...
        if(*lcd == 0 && node->GetId() == 0){
          if (m_csFromNdnSim == nullptr)
            m_cs.insert(*dataCopyWithoutTag);
          else
            m_csFromNdnSim->Add(dataCopyWithoutTag);

//          std::cout<<ti<<" forwarder::onIncomingdata(caching) "<<data.getName()<<" LcdTag is "<<*lcd<<" in Node* "<<node->GetId()<< std::endl; 
        }
...
//                         " LcdTag* set "<<*(data.getTag<lp::LcdTag>())<<" in Node* "<<node->GetId()<< std::endl; 
        }

      }else{
//        std::cout<<ti<<" forwarder::onIncomingdata(no caching) "<<data.getName()<<" LcdTag* is "<<*lcd<<" in Node "<<node->GetId()<< std::endl; 
      }

  }else{
//    std::cout<<ti<<" forwarder::onIncomingdata(no caching) "<<data.getName()<<" LcdTag is null in Node "<<node->GetId()<< std::endl; 
  }
//------------------------------------------------------------------------------------------------------------------------------------------------------
