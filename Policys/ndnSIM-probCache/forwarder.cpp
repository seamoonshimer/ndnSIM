...
// Pro (1.get ProTag | 2.ProTag++ | 3.calcul probaility | 4.cache decide)
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
   shared_ptr<lp::ProTag> pro = data.getTag<lp::ProTag>();
   auto node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
   auto ti = ns3::Simulator::Now().ToDouble (ns3::Time::S);
   if(pro !=nullptr){
     int pron = *pro;
       pron++;
       data.setTag(make_shared<lp::ProTag>(pron));
       shared_ptr<Data> dataCopyWithoutTag = make_shared<Data>(data);
       dataCopyWithoutTag->removeTag<lp::HopCountTag>();
...
//         std::cout<<ti<<" forwarder::onIncomingdata(caching) "<<data.getName()<<" ProTag is "<<*pro<<" in Node* "<<node->GetId()<< std::endl;
       }

       if(node->GetId() != 0){
         double probaility = ((6.0 - (*pro)) * 50.0 * (*pro)) / 5000.0;
         ns3::Ptr<ns3::UniformRandomVariable> m_ran(ns3::CreateObject<ns3::UniformRandomVariable>());
         double randomnum = m_ran->GetValue();
         
//         std::cout<<"Probaility: "<<probaility<<" "<<"Random num: "<<randomnum<<std::endl;
         
         if(probaility >= randomnum){
           if (m_csFromNdnSim == nullptr)
             m_cs.insert(*dataCopyWithoutTag);
           else
             m_csFromNdnSim->Add(dataCopyWithoutTag);
...
//           std::cout<<ti<<" forwarder::onIncomingdata(no caching) "<<data.getName()<<" ProTag* is "<<*pro<<" in Node "<<node->GetId()<< std::endl;
         }
