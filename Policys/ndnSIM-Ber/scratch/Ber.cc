...
static void
ReSetPrefix(const std::string& str){
 for(int j = 32; j <= 47; j++){
...
main(int argc, char* argv[])
{
  ...
  cmd.AddValue("Policy","Policy(Ber)",Policy);
  ...
  ndn::StackHelper ndnHelper;
    ...
    ndnHelper.SetOldContentStore("ns3::ndn::cs::Probability::Lru","MaxSize",CSsize,"CacheProbability","0.7"); // 边缘节点存储容量 50 概率 0.7
    //ndnHelper.SetOldContentStore("ns3::ndn::cs::Probability::Lru","MaxSize",CSsize,"CacheProbability","0.06"); // 边缘节点存储容量 50 概率 0.7
  ...
  consumerHelper.SetAttribute("q", StringValue("1.0"));                  // 设置参数q
  ...
  for(int i = 1; i < Runtime/Updata; i++){
    ...
    Simulator::Schedule(Seconds(i*Updata), &ReSetPrefix, str);
  }
