...
// Ran (1.get RanTag | 2.RanTag-- | 3.set RanTag | 4.check to cache)
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
   shared_ptr<lp::RanTag> ran = data.getTag<lp::RanTag>();
   auto node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
   auto ti = ns3::Simulator::Now().ToDouble (ns3::Time::S);
   if(ran !=nullptr){
     int rann = *ran;
       rann--;
       data.setTag(make_shared<lp::RanTag>(rann));
       shared_ptr<Data> dataCopyWithoutTag = make_shared<Data>(data);
       dataCopyWithoutTag->removeTag<lp::HopCountTag>();
...
//         std::cout<<ti<<" forwarder::onIncomingdata(caching) "<<data.getName()<<" RanTag is "<<rann<<" in Node* "<<node->GetId()<< std::endl;
       }

       if(node->GetId() != 0 && rann == 0){
         if (m_csFromNdnSim == nullptr)
           m_cs.insert(*dataCopyWithoutTag);
         else
           m_csFromNdnSim->Add(dataCopyWithoutTag);
...
//         std::cout<<ti<<" forwarder::onIncomingdata(caching) "<<data.getName()<<" RanTag* is "<<rann<<" in Node "<<node->GetId()<< std::endl;
       ...
       if(node->GetId() !=0 && rann != 0){
//         std::cout<<ti<<" forwarder::onIncomingdata(no caching) "<<data.getName()<<" RanTag* is "<<rann<<" in Node "<<node->GetId()<< std::endl;
       }
   }
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
