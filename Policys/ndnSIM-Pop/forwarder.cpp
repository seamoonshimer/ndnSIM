...
// Pop cs hit (1.set Tag 0 | 2.log)
//------------------------------------------------------------------------------------------------------------------------------------------------------
  auto node = ns3::NodeList::GetNode(ns3::Simulator::GetContext()); 
  auto ti = ns3::Simulator::Now().ToDouble (ns3::Time::S);
  data.setTag(make_shared<lp::PopTag>(0));                                                
  shared_ptr<lp::PopTag> pop = data.getTag<lp::PopTag>();
  if (pop == nullptr) {
//     std::cout<<ti<<" forwarder::cache hit "<<data.getName()<<" Node "<<node->GetId()<<" PopTag unull "<< std::endl;   
  }else{
//     std::cout<<ti<<" forwarder::cache hit "<<data.getName()<<" Node "<<node->GetId()<<" PopTag set "<<*data.getTag<lp::PopTag>()<< std::endl;   
  }
//------------------------------------------------------------------------------------------------------------------------------------------------------
...
/*
  ...
  if (m_csFromNdnSim == nullptr){
    ...
    std::cout<<"m_cs"<<std::endl;
  }
  else{
    m_csFromNdnSim->Add(dataCopyWithoutTag);
    std::cout<<"m_csFromNdnSim"<<std::endl;
  }
*/
...
// Pop 
//-----------------------------------------------------------------------------------------------------------------------------------------------------------
  shared_ptr<lp::PopTag> pop = data.getTag<lp::PopTag>();                                                           // by yin - Pop
  int inRecordNum = pitEntry->getInRecords().size();                                                        
  auto node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  auto ti = ns3::Simulator::Now().ToDouble (ns3::Time::S);

  if(pop !=nullptr){                                                                                                // Poptag null cheak
    int popn = *pop;
    
    if(popn == 0){  // No cache                                                                                     // Cached check
      
      if(inRecordNum != 2 && node->GetId() != 0){                                                                   // Cache decide
          data.setTag(make_shared<lp::PopTag>(1)); // set Poptag
          if (m_csFromNdnSim == nullptr)           // caching
            m_cs.insert(*dataCopyWithoutTag);
          else
            {m_csFromNdnSim->Add(dataCopyWithoutTag);}
//            std::cout<<ti<<" forwarder::onIncomingdata(caching) "<<data.getName()<<
//                    " InRecordNum "<<inRecordNum<<" Node "<<node->GetId()<<" set PopTag "<<*data.getTag<lp::PopTag>()<< std::endl;   
      }

      if(inRecordNum != 2 && node->GetId() == 0){
          if (m_csFromNdnSim == nullptr)           // caching
            m_cs.insert(*dataCopyWithoutTag);
          else
            {m_csFromNdnSim->Add(dataCopyWithoutTag);}
//            std::cout<<ti<<" forwarder::onIncomingdata(caching) "<<data.getName()<<
//                    " InRecordNum "<<inRecordNum<<" Node* "<<node->GetId()<<" nochange PopTag "<<*data.getTag<lp::PopTag>()<< std::endl;   
      }

      if(inRecordNum == 2 && node->GetId() != 0){
//            std::cout<<ti<<" forwarder::onIncomingdata(no caching) "<<data.getName()<<
//                    " InRecordNum* "<<inRecordNum<<" Node "<<node->GetId()<<" nochange PopTag "<<*data.getTag<lp::PopTag>()<< std::endl;   
      }
...
//            std::cout<<ti<<" forwarder::onIncomingdata(no caching) "<<data.getName()<<
//                    " InRecordNum "<<inRecordNum<<" Node "<<node->GetId()<<" PopTag null! "<<std::endl;   
  }

    // cached && InRecordNum == 2 && PopTag == null && Node == 0
    beforeSatisfyInterest(*pitEntry, inFace, data);
    // trigger strategy: after receive Data
    this->dispatchToStrategy(*pitEntry,                                                                             // send data
